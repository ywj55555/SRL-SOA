...
def loadData(dataset):
    ...
    for i in range(0, 10):
        ...
        classData['y_train'] = y_train - 1  # 训练标签值减一
        classData['y_test'] = y_test - 1  # 训练标签值减一
...
def reduce_bands(param, classData, Data, i):
    ...
    xx = classData['x_train']
    print('weights', weights)
    weights = False
    # if dataset != 'SalinasA_corrected': xx = classData['x_train']
    # else: xx = np.concatenate([classData['x_train'], Data['scd']], axis = 0)  # Data 可以不要了！！
    ...
    if modelType == 'SRL-SOA':
        ...
        if not weights:
            ...
            model.fit(xx, xx, batch_size = batchSize,  # 训练集和验证集一样，基于重构思想
        ...
        print('SRL-SOA selected: ', indices[-s_bands::])
